...
class QuestDBEngine {
  ...
  async profile(sql, params) {
    ...
    try {
      ...
      let explainText = '';
      if (this.protocol === 'http') {
        const explainUrl = `${this.baseUrl}/exec?query=${encodeURIComponent(explainSql)}`;
        ...
        // PostgreSQL protocol
        const result = await this.queryViaPg(explainSql);
      ...
      if (this.protocol === 'http') {
        const paramsUrl = `${this.baseUrl}/exec?query=${encodeURIComponent("SHOW PARAMETERS WHERE property_path ILIKE '%worker%' OR property_path ILIKE '%parallel%' OR property_path ILIKE '%jit%'")}`;
  ...
  async renderProfile(profileData, mainContainer) {
    ...
    const formattedPlan = this.formatExplainPlan(profileData.explainPlan);
